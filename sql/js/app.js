    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 pb-[85px] relative overflow-hidden">
            ...
            <div className={`shrink-0 h-1/3 bg-white border-t border-gray-200 overflow-y-auto p-4 transition-all safe-pb`}>
                {error && <div className="text-red-500 text-sm font-semibold flex items-center gap-2 animate-shake"><span className="text-lg">☕️</span> <span>Spilled! {isCleaning ? "Wait for cleanup..." : error}</span></div>}
                ...
                {!result && !error && !success && <div className="text-center text-gray-400 mt-8 text-sm flex flex-col items-center gap-2"><span className="text-2xl opacity-50">🍵</span>Waiting for the brew...</div>}
            </div>
        </div>
    );
};
