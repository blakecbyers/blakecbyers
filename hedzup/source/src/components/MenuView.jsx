...
export default function MenuView({ decks, onSelect }) {
    ...
    return (
        <div className="flex flex-col h-full safe-area-inset-bottom max-w-4xl mx-auto w-full bg-zinc-50">
            <div className="flex-none px-6 pt-16 pb-8 text-center md:text-left">
                <h1 className="text-5xl font-black tracking-tighter text-zinc-900 md:text-6xl">
                    Heads Up
                </h1>
                <h2 className="text-2xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 -mt-2">
                    Blevnecked Edition
                </h2>
                <p className="text-zinc-500 mt-4 text-lg font-medium">Select a category to begin.</p>
            </div>

            <div className="flex-1 overflow-y-auto px-6 pb-12 grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-min">
                {decks.map((deck) => (
                    <button
                        key={deck.id}
